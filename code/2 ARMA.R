...
### APSP
# Split data
ts.train <- ts(ts.apsp.monthly.absolute[1:floor(length(ts.apsp.monthly.absolute) * 0.7)], start = c(1990,1), frequency = 12)
ts.test <- ts(ts.apsp.monthly.absolute[floor(length(ts.apsp.monthly.absolute) * 0.7) + 1:length(ts.apsp.monthly.absolute)], start = c(2011, 10), end = c(2021, 2), frequency = 12)
