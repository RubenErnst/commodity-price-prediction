...
ses.results <- data.frame("commodity" = rep(c("APSP", "Brent", "Dubai", "LNG", "US NatGas", "WTI"), 2),
...
ses.results$mae[7] <- mae(ts.test, apsp.pred.log.return)
ses.results$mape[7] <- mape(ts.test, apsp.pred.log.return)
...
ses.results$mae[8] <- mae(ts.test, brent.pred.log.return)
ses.results$mape[8] <- mape(ts.test, brent.pred.log.return)
...
ses.results$mae[9] <- mae(ts.test, dubai.pred.log.return)
ses.results$mape[9] <- mape(ts.test, dubai.pred.log.return)
...
# Evaluate
lng.pred.diff <- cumsum(c(ts.lng.monthly.absolute[length(ts.lng.monthly.absolute) - 12], fit$mean))[-1]
ses.results$mae[4] <- mae(ts.test, lng.pred.diff)
ses.results$mape[4] <- mape(ts.test, lng.pred.diff)
...
# Evaluate
lng.pred.log.return <- ts.lng.monthly.absolute[length(ts.lng.monthly.absolute) - 12] * cumprod(exp(fit$mean))
ses.results$mae[10] <- mae(ts.test, lng.pred.log.return)
ses.results$mape[10] <- mape(ts.test, lng.pred.log.return)
...
ses.results$mae[5] <- mae(ts.test, natgas.us.pred.diff)
ses.results$mape[5] <- mape(ts.test, natgas.us.pred.diff)
...
# Fit SES model
fit <- ses(ts.train, h = 12) # h determines the number of periods to be forecast
ses.results$alpha[11] <- unname(fit$model$par[1])

# Evaluate
natgas.us.pred.log.return<- ts.natgas.us.monthly.absolute[length(ts.natgas.us.monthly.absolute) - 12] * cumprod(exp(fit$mean))
ses.results$mae[11] <- mae(ts.test, natgas.us.pred.log.return)
ses.results$mape[11] <- mape(ts.test, natgas.us.pred.log.return)
...
ses.results$mae[6] <- mae(ts.test, wti.pred.diff)
ses.results$mape[6] <- mape(ts.test, wti.pred.diff)
...
# Fit SES model
fit <- ses(ts.train, h = 12) # h determines the number of periods to be forecast
ses.results$alpha[12] <- unname(fit$model$par[1])

# Evaluate
wti.pred.log.return <- ts.wti.monthly.absolute[length(ts.wti.monthly.absolute) - 12] * cumprod(exp(fit$mean))
ses.results$mae[12] <- mae(ts.test, wti.pred.log.return)
ses.results$mape[12] <- mape(ts.test, wti.pred.log.return)
